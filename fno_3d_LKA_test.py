...
class Cell(nn.Module):
    ...
    def forward(self, x):
        ...
        # outputs_list = []
        outputs_list = torch.zeros(B, time_size, C, D, H, W, device=x.device)
        prev_state = torch.zeros(B, C, D, H, W, device=x.device)
        '''
        for t in range(time_size):
            ...
            if self.activation:
                ...
                outputs_list[:,t,...] = F.gelu(prev_state)
            else:
            #    prev_state = h + c
                # outputs_list.append(prev_state)
                outputs_list[:,t,...] = prev_state
        ...
        return outputs_list
        '''
        def process_timestep(t, cur_input, prev_state):
            combined = torch.cat([cur_input, prev_state], dim=1)  # Concatenate along channel dimension
            reset_gate = torch.sigmoid(self.reset_gate(combined))
            update_gate = torch.sigmoid(self.update_gate(combined))
            combined_reset = torch.cat([cur_input, reset_gate * prev_state], dim=1)
            out_gate = torch.tanh(self.out_gate(combined_reset))
            prev_state = update_gate * prev_state + (1 - update_gate) * out_gate

            # Process current input
            c_x1 = self.conv_x(cur_input)
            c_x1 = self.mlp_x(c_x1)
            c_x2 = self.w_x(cur_input)

            if self.skip:
                c = c_x1 + c_x2 + cur_input
            else:
                c = c_x1 + c_x2
            c = F.gelu(c)
            ...
            if self.activation:
                return F.gelu(prev_state), prev_state
            else:
                return prev_state, prev_state
            
        for t in range(time_size):
            cur_input = x[:, t, ...]
            if self.training and t % 2 == 0:
                # Apply checkpoint every 2 time steps for better memory efficiency
                out, prev_state = checkpoint(lambda *args: process_timestep(*args), t, cur_input, prev_state)
            else:
                out, prev_state = process_timestep(t, cur_input, prev_state)
...
class FNO3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width, input_channel=1, num_cell=4, skip=False, periodic=False):
        ...
        self.cell_1 = Cell(modes1, modes2, modes3, width, self.skip)
        self.cell_2 = Cell(modes1, modes2, modes3, width, self.skip)
        self.cell_3 = Cell(modes1, modes2, modes3, width, self.skip)
        self.cell_4 = Cell(modes1, modes2, modes3, width, self.skip, activation=False)
        self.cells = nn.Sequential(self.cell_1, self.cell_2, self.cell_3, self.cell_4)
    ...
    def forward(self, x):
        ...
        #     x = self.cell_1(x)
        #     x = checkpoint(self.cell_2, x)
        #     x = self.cell_3(x)
        #     x = checkpoint(self.cell_4, x)
